      ...
      if (dataResults.length == dataUrls.length) {
        ...
        for (i = 0; i < dataResults.length; i++) {
          ...
          function moveBig() {
            bubble.each(function (){
              var coordinates = $(this).data("coordinates")
              $(this).animate({left: coordinates.left + Math.random()*250}, {easing: "swing", queue: false, duration: 5000});
              $(this).animate({top: coordinates.top + Math.random()*50}, {easing: "swing", queue: false, duration: 5000});
            });
